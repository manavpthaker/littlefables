...
export function GateForm({ nextPath }: { nextPath: string }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(body.error === 'wrong_password' ? 'That password did not match.' : 'Could not sign in.');
        return;
      }
      // Hard nav so the cookie is present on the RSC that renders the target.
      window.location.href = nextPath || '/parent';
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <form onSubmit={onSubmit} style={{ display: 'grid', gap: 'var(--space-2)' }}>
      ...
      {error && (
        <p style={{ margin: 0, color: 'var(--danger, #c94a3b)', fontSize: 'var(--text-caption)' }}>
          {error}
        </p>
      )}
      <button
