...
export function StoryText({ words, currentIndex = -1, starredWords = [], keptWords = [], onHearWord, onStarWord, overArt = false }) {
  ...
  return (
    <p style={{
      fontFamily: 'var(--font-display)', fontSize: 'var(--text-reading)', lineHeight: 'var(--lh-reading)',
      ...
      {words.map((t, i) => {
        ...
        const kept = keptWords.includes(stem(t.w));
        return (
          <span key={i} style={{ position: 'relative', whiteSpace: 'nowrap' }}>
            <span role="button" tabIndex={0} data-utterance={tapped === i ? `Keep ${t.w}?` : t.w}
              ...
              style={{
                ...
                margin: '.14em 0',
                background: state === 'current' ? 'var(--word-current-bg)'
                  : tapped === i ? 'var(--marigold-wash)'
                  : kept ? 'rgba(110,154,87,.32)'
